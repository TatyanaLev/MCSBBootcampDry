...
cell_metadata <- read.table("/Users/x2127151/Desktop/BootCamp_dry/MCSBBootcampDry/Zoo/R/cell_metadata.txt")

#extract the normalized data matrix
expression_matrix <- readRDS("/Users/x2127151/Desktop/BootCamp_dry/MCSBBootcampDry/Zoo/R/expression_matrix.rds")
```
...
most_expressed <- n_cells_expressed[which(n_cells_expressed %in% max(n_cells_expressed))]  #%which value matches the max value
...
```{r q8}
ggplot(average_mki67_by_cluster_and_condition, mapping = aes(x=res.0.4, y=mean_mki67, fill=orig.ident)) + geom_bar(stat="identity", position="dodge") #dodge to unstack the barss from on top of each other
```
...
matrix_subset_of_interest <- as.matrix(t(genes_of_interest_df)) # t is transpose the matrix
...
```{r cell cycle genes}
cell_cycle_genes <- as.character(read.table("/Users/x2127151/Desktop/BootCamp_dry/MCSBBootcampDry/Zoo/R/regev_lab_cell_cycle_genes.txt", header=F)$V1)
```
